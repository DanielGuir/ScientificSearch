...
class Query(val querySentence: String, var debug: Boolean = false, var printQuery: Boolean = false) {
  if (printQuery) {println(s"Query Sentence: $querySentence")}
  ...
  def getAnchorCapture(proc: Processor){
    var idx: Int = 0
    // find record anchor & capture location
    val doc_pre = proc.annotate(querySentence)
    var offset = 0
    for ((word, idx) <- doc_pre.sentences(0).words.zipWithIndex){
      if (word == "$"){
        anchors += idx - offset
        offset += 1
      } else if (word == ":"){
        queryCaptures += idx - offset
        offset += 1
      }
    }
    // remove anchor & capture mark
    ...
    if(debug){
      println(s"Anchors $anchors")
      println(s"Captures $queryCaptures")
    }
  }
  /** wrapper function for all query pre-processing * */
  def preProcess(proc: Processor) {
    getAnchorCapture(proc)
    getRawGraph(proc)
  ...
  def generateResult(
    ...
    val resultCaptures = ListBuffer[ListBuffer[String]]()
    ...
    while (curID < scoreDocs.length) {
      ...
      if (longestPath.length == queryGraph.size) {
        if (matchID % numToDisplay == 1 & matchID > 1 & numToDisplay != -1) {
          return (resultText, resultDoc, resultCaptures, capturesBuilder)
        }
        matchID += 1
        // filter out those not in capture
        for (c <- captures) {
          // print(s"original id${c.name}")
          val nodeID = c.name.substring(4).toInt
          // print(s"original nodeid${nodeID}")
      ...
      // display results
      val res = extractorEngine.index.doc(hit.doc).getField("raw").stringValue
      ...
      breakable {
        for (c <- chunkedCapture) {
          ...
          //debug
            ...
            // print(s"start${c.capturedMatch.start}")
            // print(s"end${c.capturedMatch.end}")
          if (c.label == None) {
            val id = c.name
            // build single capture word (span)
            val singleCapWordBuilder = ArrayBuffer[String]()
            for (i <- c.capturedMatch.start until c.capturedMatch.end) {
              if (i >= resultTokens.length) {
                singleCapWordBuilder.append("???")
              } else {
            ...
            //   }
            // }
            // if finish, add to count. Check required because one capture might match to multiple words
            // if (finish) {
              // val capWordsString = capWordsBuilder.mkString("$")
              // val itemToAdd = (capturesString, capWordsString)
              // re-initialize
              // resultCaptures += capWordsBuilder.clone
              // break
              // FIX: only include one set of captures
    ...
    assert(resultText.length == resultCaptures.length)
  ...
  def getChunk(doc: LuceneDocument, cap: NamedCapture): NamedCapture = {
    ...
    if (startChunk != "O") {
      ...
      // find start of chunk
      while (start > 0 && chunks(start).split("-")(0) != "B") {
        start -= 1
      }
      // find end of chunk
  ...
  def convertSentenceToGraph(
    ...
    for ((token, idx) <- doc.sentences(0).words.zipWithIndex){
      idx2token += (idx -> token)
    }
    // // record token id
